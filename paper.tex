...
below a safety target is understated by exactly the factor $D$, which amends the
standard mileage calculation in the direction of premature confidence. Third,
...
Almost all of this work rests on a modelling assumption that is rarely stated and,
to our knowledge, never tested: that the safety-relevant events logged by a fleet
are \emph{independent}. Terres et al.~\cite{terres2023behavioral} model the count
...
The independence assumption is convenient and, in this setting, wrong in a
specific and consequential way. Safety-relevant events arrive in clusters. A
...
using it, the quantification of what its absence costs in the regime AV validation
actually operates in, and the observation that the correction requires no new
instrumentation.
...
clustering, serial dependence, or effective sample size; two of the three
explicitly flag dependence as out of scope.
...
and cluster-robust variance estimators are built for, and applying a point-process
excitation model to it would be a misspecification of a different sort.
...
and sequential bootstrapping as remedies. The present paper is, in effect, the
observation that these adjacent literatures have solved a problem that AV fleet
rate estimation currently does not acknowledge having.
...
$D = 3$, and $54.1\%$ at $D = 7$. At the upper end of that range a stated
``$95\%$ confidence interval'' is closer to a coin flip than to its label.
...
This is the practically important statement. A programme that computes its
required mileage assuming independence, and stops when it reaches that figure, has
collected between one third and one seventh of the evidence it believes it has, for
design effects in the range studied here. The error is in the unsafe direction: it
produces early declarations of demonstrated safety, not late ones.
...
an unremarkable figure for fleet data --- already reduces a nominal $95\%$
...
\paragraph{What this establishes.} That the independence assumption in current AV
fleet rate estimation is not innocuous; that its cost is a specific, computable
...
\paragraph{Relationship to existing practice.} Nothing here suggests that fleets
are not aware that their data is structured. Engineering teams routinely analyse
events by scene, by route, and by release. The claim is narrower and, we think,
harder to dismiss: that this structure is not currently propagated into the
published confidence statements and mileage targets, and that propagating it
changes those statements materially.
